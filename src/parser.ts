...
const mergeMarkText = <M extends string>(marks: Mark<M>[]): string =>
    marks?.reduce(
        (a, m) =>
            a + ("text" === m.name ? m.content : m.unbreakable ? BEGIN + END : BEGIN + mergeMarkText(m.children) + END),
        ""
    ) || "";
    ...
    return text
        ...
        .reduce(
            (s, t, i) =>
                i % 2
                    ? t.startsWith(BEGIN)
                        ? s.addProcessedMarks(t.length)
                        : s.closeProcessedMarks(t.length)
                    : s.addText(t),
            state
        );
};
